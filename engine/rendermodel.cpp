...
void playeralpha(const char *a, dynent *d, modelattach *attachments, const char *mdlname, int hold, float fade){
	if(a[0]){
		fade = min(fade, (float)atoi(a));
		rendermodel(NULL, mdlname, hold ? hold : ANIM_IDLE|ANIM_LOOP, d->feetpos(), testanims && d==player ? 0 : d->yaw+90, testpitch && d==player ? testpitch : d->pitch, MDL_LIGHT, d, attachments, 0, 0, fade);
	}else{
		conoutf("playeralpha = %d", (float)atoi(a));
	}
}
COMMAND(playeralpha, "s");
//EZMod*/
