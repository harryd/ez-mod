...
namespace game
{
	...
	//EZMod
	void kickamount(const char *a, const char *b, fpsent *d, const vec &targ){
		vec unitv;
        vec kickback(unitv.div(targ.dist(d->o, unitv)));
		if (a[0]) {
			if(b[0]){
				if ((int)atoi(a)<0) {
					conoutf("Valid range for kickamount weapon is 0..6");
					guns[0].kickamount=(int)atoi(b);
				}else if ((int)atoi(a)>6) {
					conoutf("Valid range for kickamount weapon is 0..6");
					guns[6].kickamount=(int)atoi(b);
				}else {
					guns[(int)atoi(a)].kickamount=(int)atoi(b);
				}
				kickback.mul(guns[d->gunselect].kickamount*-2.5f);
			}else{
				if ((int)atoi(a)<0) {
					conoutf("Valid range for kickamount weapon is 0..6");
					conoutf("%s kickamount = %d", guns[0].name, guns[0].kickamount);
				}else if ((int)atoi(a)>6) {
					conoutf("Valid range for kickamount weapon is 0..6");
					conoutf("%s kickamount = %d", guns[6].name, guns[6].kickamount);
				}else {
					conoutf("%s kickamount = %d", guns[(int)atoi(a)].name, guns[(int)atoi(a)].kickamount);
				}
			}
		}else if(guns[0].kickamount==0&&guns[1].kickamount==20&&guns[2].kickamount==7&&guns[3].kickamount==10&&guns[4].kickamount==30&&guns[5].kickamount==10&&guns[6].kickamount==7){
			conoutf("Current values are 0, 20, 7, 10, 30, 10, 7");
		}
		else{
			conoutf("Default values are 0, 20, 7, 10, 30, 10, 7;\nCurrent values are %d, %d, %d, %d, %d, %d, %d", guns[0].kickamount, guns[1].kickamount, guns[2].kickamount, guns[3].kickamount, guns[4].kickamount, guns[5].kickamount, guns[6].kickamount);
		}
	}
	COMMAND(kickamount, "ss");
	
	void range(const char *a, const char *b, fpsent *d, const vec &targ){
		float shorten = 0;
		if (a[0]) {
			if(b[0]){
				if ((int)atoi(a)<0) {
					conoutf("Valid range for range weapon is 0..6");
					guns[0].range=(int)atoi(b);
				}else if ((int)atoi(a)>6) {
					conoutf("Valid range for range weapon is 0..6");
					guns[6].range=(int)atoi(b);
				}else {
					guns[(int)atoi(a)].range=(int)atoi(b);
				}
				shorten = guns[d->gunselect].range;
			}else{
				if ((int)atoi(a)<0) {
					conoutf("Valid range for range weapon is 0..6");
					conoutf("%s range = %d", guns[0].name, guns[0].range);
				}else if ((int)atoi(a)>6) {
					conoutf("Valid range for range weapon is 0..6");
					conoutf("%s range = %d", guns[6].name, guns[6].range);
				}else {
					conoutf("%s range = %d", guns[(int)atoi(a)].name, guns[(int)atoi(a)].range);
				}
			}
		}else if(guns[0].range==14&&guns[1].range==1024&&guns[2].range==1024&&guns[3].range==1024&&guns[4].range==2048&&guns[5].range==1024&&guns[6].range==1024){
			conoutf("Current values are 14, 1024, 1024, 1024, 2048, 1024, 1024");
		}
		else{
			conoutf("Default values are 14, 1024, 1024, 1024, 2048, 1024, 1024;\nCurrent values are %d, %d, %d, %d, %d, %d, %d", guns[0].range, guns[1].range, guns[2].range, guns[3].range, guns[4].range, guns[5].range, guns[6].range);
		}
	}
	COMMAND(range, "ss");
