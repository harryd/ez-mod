...
namespace game
{
    ...
    ICOMMAND(triggerbot, "s", (const char *a), {bTriggerBot=atoi(a);});
    /// End Of Trigger Bot
    ...
    int selectcrosshair(float &r, float &g, float &b)
    {
        ...
        int crosshair = 0;
        if(lasthit && lastmillis - lasthit < hitcrosshair) {crosshair = 2;r=b=0;}
		...
		/// Trigger Bot
        bool Attack = false;
        dynent *deNearestPlayer = intersectclosest(d->o, worldpos, d);
        if(deNearestPlayer && deNearestPlayer->type == ENT_PLAYER && !isteam(((fpsent *)deNearestPlayer)->team, d->team)) {
